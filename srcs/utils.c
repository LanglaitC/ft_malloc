...
/*   Updated: 2019/11/13 14:37:36 by clanglai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_zone_info allocate_zone(size_t size, t_zone_info zone_info) {
    if (size < TINY) {
...
t_info  *get_info_variable(size_t size) {
    ...
    if (size < TINY && !global_info->small) {
        global_info->small_info = allocate_zone(size, global_info->small_info);
        global_info->small = global_info->small_info.start;
        global_info->small->address = global_info->small + sizeof(t_alloc);
        global_info->small->status = NOALLOC;
    } else if (size >= TINY && size < SMALL && !global_info->medium) {
        allocate_zone(size, global_info->medium_info);
        global_info->medium = global_info->small_info.start;
        global_info->medium->address = global_info->small + sizeof(t_alloc);
        global_info->medium->status = NOALLOC;
    }
    return global_info;
}